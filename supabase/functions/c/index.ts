...
router.post("/:qrToken/addons", async (req, { qrToken }) => {
  ...
  return await idempotent(req, "c/addons", async (raw) => {
    ...
    //
    // ตรงนี้จงใจไม่ตรวจกฎเงินเอง ปล่อยให้ trigger เป็นด่านเดียว (§01) ซึ่งปฏิเสธสองกรณี
    // คือจำนวนเกินจำนวนหัวที่จ่ายเงิน และการ "ลด" จำนวนลงจากค่าเดิม (BR-03 ที่ 0018 เติม)
    // กรณีหลังคือช่องที่สั่งรีฟิลแล้วดื่มหมด แล้วส่ง qty เป็นศูนย์ทับก่อนเช็กบิล
    // ข้อความของ trigger ถูกแปลงเป็น 422 ให้เองที่ fromPostgres() จึงถึงหน้าจอเป็นภาษาไทย
    const row = unwrap(
      await admin
        .from("visit_addon")
        .upsert(
